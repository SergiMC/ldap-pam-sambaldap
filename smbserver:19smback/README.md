...
Per tal de tenir la configuració del samba amb LDAP com a backend correctament, haurem de configurar i assegurar-nos:
  ...
  - EL servidor ldap inclou l'schema sambaSergi.schema.
...
docker run --rm --name host --hostname host --network sambanet -it --privileged sergimc/hpamsmb:19smb

docker run --rm --name samba --hostname samba --network sambanet -it --privileged sergimc/serversmb:19smb

```

#### Verificacions
Per verificar que hi ha connexions entre LDAP i SAMBA fem aquestes ordres:
...
- En aquest fitxer de configuració em d'editar aquests apartats i posar:
  - slaveLDAP="ldap://172.20.0.4/"
  - masterLDAP="ldap://172.20.0.4/"
  - ldapTLS="0"
...
SID for local machine SAMBA is: S-1-5-21-3630333718-3621459654-1328528241
SID for domain SAMBA is: S-1-5-21-3630333718-3621459654-1328528241
